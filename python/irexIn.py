...
#赤外線データ学習
def learn_irData(irser, fileName):   
    verCmd = 0x02
    buf = []    

    buf.append(verCmd)                              #学習モード開始コマンド
    buf.append(0x00)                                #通常モード
    ...
    #終了コード確認
    if payload_data[1] == 0x02:
        print("学習が正常に終了しました:0x%X" % payload_data[1])
    elif payload_data[1] == 0x03:
        print("タイムアウトしました(15秒):0x%X" % payload_data[1])
        irser.close()
        sys.exit()  #プログラム終了
    elif payload_data[1] == 0x04:
        print("オーバーフローしました:0x%X" % payload_data[1])
        irser.close()
        sys.exit()  #プログラム終了
    else:
    ...
    #赤外線学習データバイト数取得
    ir_data_len = (((payload_data[3]<<8) & 0xFF00) + payload_data[4])

    ir_signal_data = []
    for i in range(ir_data_len):
        ir_signal_data.append(payload_data[i+5])

    #JSONデータ作成
    ir_data = {"FormatType":payload_data[2], 
               "DataLength":ir_data_len, 
               "SignalData":ir_signal_data}
...
def get_fwVersion(irser):   
    ...
    output_irSerial(buf, irser)    #コマンド送信
    payload_data = input_irSerial(verCmd, irser) #データ受信
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--file', metavar='[irDataFile]', nargs=1, help='赤外線信号の学習開始。学習した赤外線信号を保存するファイルを指定 例)-f irData.json')
    ...
    #シリアル通信設定
    try:
        ser = serial.Serial(dev_file, 115200, timeout=15)
    except:
        print("*** シリアルの設定に失敗しました")
        ser.close()
        sys.exit()
    ...
    if args.file:  #赤外線リモコンデータ学習
        print('赤外線リモコンデータを学習します')        
        learn_irData(ser, args.file[0])
